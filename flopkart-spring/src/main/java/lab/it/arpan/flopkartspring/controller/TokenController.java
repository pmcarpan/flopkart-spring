...
import lab.it.arpan.flopkartspring.model.request.TokenObtainRequest;
...
public class TokenController {
    ...
    public ResponseEntity<Map<String, String>> obtainToken(@RequestBody TokenObtainRequest tokenObtainRequest) {
        if (!tokenObtainRequest.getUsername().equals("arpan") || !tokenObtainRequest.getPassword().equals("password")) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Invalid username/password.");
        }

        User user = User.of(1L, "arpan", "password");
